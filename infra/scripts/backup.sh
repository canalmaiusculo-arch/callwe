...
# Usa AWS CLI via Docker (não requer sudo/apt).
#
# Configurar cron:
#   crontab -e
...
# AWS CLI via Docker — imagem oficial da Amazon
aws_docker() {
  docker run --rm \
    -v /tmp:/tmp \
...
aws_docker --endpoint-url "${S3_ENDPOINT}" s3 cp "${TMPFILE}" "s3://${S3_BUCKET}/backups/callwe-${STAMP}.sql.gz" --quiet
...
aws_docker --endpoint-url "${S3_ENDPOINT}" s3api list-objects-v2 \
  --bucket "${S3_BUCKET}" \
  --prefix "backups/" \
...
" | while read -r key; do
  [ -z "$key" ] && continue
  echo "   deletando $key"
  aws_docker --endpoint-url "${S3_ENDPOINT}" s3 rm "s3://${S3_BUCKET}/${key}" --quiet
done
